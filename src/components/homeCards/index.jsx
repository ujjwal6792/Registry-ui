...
const HomeCards = (props) => {
  const {id, name, updateDate, description, onClick} = props
  ...
  return (
    <div key = {id} onClick={onClick}
        className="cursor-pointer grid gap-4 border rounded shadow-md bg-grey-extra-light hover:scale-105 hover:transition-transform">
          <div className="p-4">
          <h6 className="py-2">{name}</h6>
          <p className="font-light text-xs text-grey-text">Last Updated: { updateDate|| dateFormat.format(today)}</p>
          </div>
